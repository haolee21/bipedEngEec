...
if(q1(1)>pi/2)
    tau_tend_1 = [p.ank_stiff*(pi/2-q1(1)),0,0,0,0,0];
    dtau_tend_1(1,1) = -p.ank_stiff;
end

if(q1(6)<-3*pi/2)
    tau_tend_2 = [0,0,0,0,0,p.ank_stiff*(-3*pi/2-q1(6))];
    dtau_tend_2(6,6) = -p.ank_stiff;
end


out_t = (u1-V-G1+tau_ext_toe+tau_ext_heel-dq1*eye(p.numJ)*p.joint_fri+tau_tend_1+tau_tend_2)/M;
...
grad_t =(dTaudx-dVdx-dGdx-dMdx+dTau_ext_toe+dTau_ext_heel-[zeros(p.numJ);p.joint_fri*eye(p.numJ);zeros(p.numJ)]+dtau_tend_1+dtau_tend_2)/M;
