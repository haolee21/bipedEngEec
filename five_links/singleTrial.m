...
%   --> Derive_Equations.m 
%
function result = singleTrial(stepLength,stepTime,kneDir,iterTime)
...
problem.func.pathObj = @(t,x,u)( obj_torqueSquared(u,x) ); %add state into path constraints (limit q3 velocity)

problem.func.bndCst = @(t0,x0,tF,xF)( stepConstraint(x0,xF,param) );

problem.func.pathCst = @(t,x,u)( pathConstraint(x,kneDir) );
...
% have no idea why it disable the stance ankle not the swing ankle
% problem.bounds.control.low(5) = 0;
% problem.bounds.control.upp(5) = 0;
...
q0 = [...
    0; % stance leg tibia angle
    0; % stance leg femur angle
    0; % torso angle
    0; % swing leg femur angle
    0]; % swing leg tibia angle
%q0 = randn(5,1)*15/180*pi;
...
% problem.options(1).nlpOpt = optimset(...    % turn off the display
%     'Display','off',...   % {'iter','final','off'}
%     'TolFun',1e-3,...
%     'MaxFunEvals',1e4);   %options for fmincon
problem.options.nlpOpt = optimset(...    % turn off the display
    'Display','final',...   % {'iter','final','off'}
    'TolFun',1e-6,...
    'MaxFunEvals',iterTime,...   %options for fmincon
    'MaxIter',iterTime);
switch method
...
result.u = u;
result.param=param;
result.gaitLength = param.l1*(sin(q(5,end))-sin(q(1,end)))+param.l2*(sin(q(4,end))-sin(q(2,end)));
if result.gaitLength <0.01 % in case gait length <0
    result.gaitLength = 1e-8;
