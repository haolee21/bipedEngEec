...
q = [pi/2*ones(1,length(time))+randn(1)*0.01*pi/2;
     -pi/2*ones(1,length(time))+randn(1)*0.01*pi/2;
     zeros(1,length(time))+randn(1)*0.01*pi/2;
     -pi*ones(1,length(time))+randn(1)*0.01*pi/2;
     pi/2*ones(1,length(time))+randn(1)*0.01*pi/2];
dq = zeros(param.numJ,length(time))+randn(1)*0.1*pi/2;


u = zeros(param.numJ,length(q))+randn(1)*0.1*pi/2;
...
disp(['gradient err:',string(norm(err)/norm(c2-c1))]);
...
grad_V = dV_dx(x1_row(6),x1_row(7),x1_row(8),x1_row(9),x1_row(10),x1_row(2),x1_row(3),x1_row(4),x1_row(5));

err = diff_V - grad_V.'*dx_row.';
...
grad_G = dG_dx(x1_row(1),x1_row(2),x1_row(3),x1_row(4),x1_row(5));

err = diff_G.' - grad_G*dx_row.';
...
err = out_t2-out_t1-dx_row*grad_t1;
...
err = out_t2-out_t1-dx_row*grad_t1-dx_row_2*grad_t_k1;
